...
static doca_error_t
sample_drop_callback(void *param, void *config_voidp)
{
	struct geneve_demo_config * config = config_voidp;
	config->drop_samples = *(bool*)param;
	DOCA_LOG_INFO("Drop samples: %s", config->drop_samples ? "true" : "false");
	return DOCA_SUCCESS;
}
...
geneve_demo_register_argp_params(void)
{
	...
	if (ret != DOCA_SUCCESS)
		DOCA_LOG_ERR("Failed to create ARGP param: %s", doca_error_get_descr(ret));
	doca_argp_param_set_short_name(param, "sd");
	doca_argp_param_set_long_name(param, "sample-drop");
	doca_argp_param_set_description(param, "Indicates sampled packets should be dropped on xmit");
	doca_argp_param_set_callback(param, sample_drop_callback);
